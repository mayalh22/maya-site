...
export default function SocialGrid({ social: initialSocial }) {
  ...
  const { items, patchItem, createItem, deleteItem, reorder } = useOwnerCollection('social', {
      ...
      <div className="contact-grid">
        {list.map((link, index) => {
          ...
          return (
            <div
              key={link.id}
              className={`card ${shapeClassName(link.shape, index)} ${dragIndex === index ? 'dragging' : ''}`.trim()}
