...
export default function ProjectsGrid({ projects: initialProjects }) {
  ...
  const { items, patchItem, createItem, deleteItem, reorder } = useOwnerCollection('projects', {
      ...
      <div className="projects-grid">
        {list.map((project, index) => {
          ...
          return (
            <div
              key={project.id}
              className={`card ${shapeClassName(project.shape, index)} ${dragIndex === index ? 'dragging' : ''}`.trim()}
